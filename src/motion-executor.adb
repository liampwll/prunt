      ...
      for I in Working.Feedrate_Profiles'Range loop
         declare
            Start_Curve_Half_Distance : constant Length :=
              Distance_At_T (Working.Beziers (I - 1), 1.0) - Distance_At_T (Working.Beziers (I - 1), 0.5);
            End_Curve_Half_Distance   : constant Length := Distance_At_T (Working.Beziers (I), 0.5);
            Mid_Distance              : constant Length :=
              abs (Point_At_T (Working.Beziers (I), 0.0) - Point_At_T (Working.Beziers (I - 1), 1.0));
         begin
            while Current_Time < Total_Time (Working.Feedrate_Profiles (I)) loop
               ...
               begin
                  if Distance < Start_Curve_Half_Distance then
                     Point := Point_At_Distance (Working.Beziers (I - 1), Distance + Start_Curve_Half_Distance);
                  elsif Distance < Start_Curve_Half_Distance + Mid_Distance then
                     Point :=
                       Point_At_T (Working.Beziers (I - 1), 1.0) +
                       (Point_At_T (Working.Beziers (I), 0.0) - Point_At_T (Working.Beziers (I - 1), 1.0)) *
                         ((Distance - Start_Curve_Half_Distance) / Mid_Distance);
                  else
                     Point :=
                       Point_At_Distance (Working.Beziers (I), Distance - Start_Curve_Half_Distance - Mid_Distance);
                  end if;
